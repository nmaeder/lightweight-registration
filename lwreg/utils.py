...
def configure_from_database(
    ...
    if connection is not None:
        ...
        raise ValueError('at least one of dbname or connection must be provided')
...
def _process_molblock(molblock, config):
    mol = Chem.MolFromMolBlock(molblock, sanitize=False, removeHs=_lookupWithDefault(config, 'removeHs'))
    return mol
...
def hash_mol(mol, escape=None, config=None):
    ...
    layers = RegistrationHash.GetMolLayers(
        mol,
        escape=escape,
        enable_tautomer_hash_v2=_lookupWithDefault(config, 'useTautomerHashv2'),
    )
...
def _register_one_conformer(mrn, sMol, molb, cn, curs, config, fail_on_duplicate, confId=-1):
    try:
        chash = _get_conformer_hash(sMol, _lookupWithDefault(config, 'numConformerDigits'), confId=confId)
        regtuple = (mrn, chash, molb)
        qs = '?,?,?'
        if _dbtype != 'postgresql':
            curs.execute(
                _replace_placeholders(f'insert into {conformersTableName} values (NULL,{qs})'),
                regtuple,
            )
            ...
            curs.execute(
                _replace_placeholders(f'insert into {conformersTableName} values (default,{qs}) returning conf_id'),
                regtuple,
            )
            conf_id = curs.fetchone()[0]
        cn.commit()
        ...
        if fail_on_duplicate:
            ...
            curs.execute(
                _replace_placeholders(f'select conf_id from {conformersTableName} where conformer_hash=?'),
                (chash,),
            )
            conf_id = curs.fetchone()[0]
    except:
...
def _register_mol(
    ...
    if registerConformers and tpl.mol is not None and not tpl.mol.GetNumConformers():
        raise ValueError('attempt to register a molecule without conformers when registerConformers is set')
    ...
    if def_std_label is None:
        curs.execute(f"select value from {registrationMetadataTableName} where key='standardization'")
    ...
    if def_rdkit_version_label is None:
        curs.execute(f"select value from {registrationMetadataTableName} where key='rdkitVersion'")
    ...
    try:
        ...
        if _dbtype != 'postgresql':
            curs.execute(
                _replace_placeholders(f'insert into {hashTableName} values (NULL,{qs})'),
                regtuple,
            )
            curs.execute('select last_insert_rowid()')
            mrn = curs.fetchone()[0]
            ...
            #  i.e. failed inserts will increment the counter
            curs.execute(
                _replace_placeholders(f'insert into {hashTableName} values (default,{qs}) returning molregno'),
                regtuple,
            )
            mrn = curs.fetchone()[0]
        ...
        cn.rollback()
        if fail_on_duplicate and not (registerConformers and sMol.GetNumConformers()):
            raise
        else:
            curs.execute(
                _replace_placeholders(f'select molregno from {hashTableName} where fullhash=?'),
                (mhash,),
            )
            mrn = curs.fetchone()[0]
    except:
    ...
    if registerConformers and sMol.GetNumConformers():
        conf_id = _register_one_conformer(mrn, sMol, molb, cn, curs, config, fail_on_duplicate, confId=confId)
...
def _get_mols_from_smilesfile(smilesfile):
    ...
    if not has_header:
        ...
        mols = Chem.SmilesMolSupplierFromText(content, delimiter=delimiter, smilesColumn=col)
    else:
        mols = Chem.SmilesMolSupplier(smilesfile, delimiter=delimiter, smilesColumn=col)
    return mols
...
def register(
    ...
    tpl = _parse_mol(mol=mol, molfile=molfile, molblock=molblock, smiles=smiles, config=config)
    ...
    mrn, conf_id = _register_mol(tpl, escape, cn, curs, config, fail_on_duplicate, confId=confId)
    if mrn is None:
...
def register_multiple_conformers(config=None, mol=None, escape=None, fail_on_duplicate=True, no_verbose=True):
    ...
    _check_config(config)
    if not _lookupWithDefault(config, 'registerConformers'):
        raise ValueError('register_multiple_conformers can only be used when registerConformers is enabled')
    ...
    for i, conf in enumerate(sMol.GetConformers()):
        ...
        mrn = confMrns[i]
        conf_id = _register_one_conformer(mrn, sMol, molb, cn, curs, config, fail_on_duplicate, confId=conf.GetId())
        res.append((mrn, conf_id))
...
def query(
    ...
    if ids is not None:
        if not _lookupWithDefault(config, 'registerConformers'):
            raise ValueError('passing ids to query() only makes sense when registerConformers is enabled')
        ...
        cn = connect(config)
        curs = cn.cursor()
        if not _lookupWithDefault(config, 'registerConformers') or not sMol.GetNumConformers():
            mhash, hlayers = hash_mol(sMol, escape=escape, config=config)
            if layers == 'ALL':
                queryText = _replace_placeholders(f'select molregno from {hashTableName} where fullhash=?')
                queryVals = (mhash,)
            else:
                vals = []
                query = []
            ...
            chash = _get_conformer_hash(sMol, _lookupWithDefault(config, 'numConformerDigits'))
            curs.execute(
                _replace_placeholders(f'select molregno,conf_id from {conformersTableName} where conformer_hash=?'),
                (chash,),
            )
...
def retrieve(config=None, ids=None, id=None, as_submitted=False, as_hashes=False, no_verbose=True):
...
def _registerMetadata(curs, config):
    ...
    for k, v in dc.items():
        if k in ('connection', 'user', 'password'):
            continue
        curs.execute(
            _replace_placeholders(f'insert into {registrationMetadataTableName} values (?,?)'),
            (str(k), str(v)),
        )
